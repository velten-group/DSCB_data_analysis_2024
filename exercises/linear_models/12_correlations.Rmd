...
3. Calculate the Pearson and the Spearman correlation across all countries between the 4 variables CHOCOLATE, NOBEL, POPULATION, INTERNET using the `cor` function. With the `method` argument you can specify the type of correlation you want to calculate, e.g. `pearson`, `spearman`. Which pair of variables has the highest correlation? 
```{r}
# to be filled
```
